//	gr; from https://www.shadertoy.com/view/4sKyzW
#define vec3 float3
#define vec2 float2
#define vec4 float4
...
// Modified from http://tog.acm.org/resources/GraphicsGems/gems/Roots3And4.c
// Credits to Doublefresh for hinting there
int solve_quartic(vec4 coeffs, out vec4 s){
    ...
    //}
    {
		...
		vec2 tmp=vec2(FLOAT_MAX,FLOAT_MAX);
		int old_num=num;
...
vec4 roots_drv = vec4(FLOAT_MAX,FLOAT_MAX,FLOAT_MAX,FLOAT_MAX);
s = roots_drv;	
		...
		if(i < num)
{
		float4 temps = s[i];
			temps -= sub;
			temps = halley_iteration4(coeffs,temps);
s[i] = temps;
...
float cubic_bezier_sign(vec2 uv, vec2 p0, vec2 p1, vec2 p2, vec2 p3){
	...
	vec3 roots = vec3(FLOAT_MAX,FLOAT_MAX,FLOAT_MAX);
...
float cubic_bezier_dis(vec2 uv, vec2 p0, vec2 p1, vec2 p2, vec2 p3)
{
	...
	vec3 a = vec3(FLOAT_MAX,FLOAT_MAX,FLOAT_MAX);
	vec3 b = vec3(FLOAT_MAX,FLOAT_MAX,FLOAT_MAX);
	...
	if(num_roots_drv==4)
{
		...
		if(num_roots_drv==2){
			...
			vec3 roots_snd_drv=vec3(FLOAT_MAX,FLOAT_MAX,FLOAT_MAX);
			int num_roots_snd_drv=solve_cubic(c2,roots_snd_drv);

			vec2 roots_trd_drv=vec2(FLOAT_MAX,FLOAT_MAX);
		...
		for(int i=0;i<3;i++){
			if(i < num_roots){
				...
				for(int k=0;k<2;k++){
					if(k < num_roots_trd_drv){
						if(a[i] < roots_trd_drv[k] && b[i] > roots_trd_drv[k]){
							if(eval_poly5(b0,b1,b2,b3,b4,roots_trd_drv[k]) > 0.){
								b[i]=roots_trd_drv[k];
							}
							else{
								a[i]=roots_trd_drv[k];
